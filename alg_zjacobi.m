function [V, success, i, FNorm, eNorm, p, mu] = alg_zjacobi(baseMVA, bus, gen, branch, opt, optAn, Vsol)
...
V0 = ones(length(V0),1)*(1+1i*0); % REMOVE
...
while (~success && i < max_it)
  ...
  Vnew([pv; pq]) = V([pv; pq]) + alpha * dV([pv; pq]);
  % Correct voltage magnitude back to setpoint values for pv buses
  Vnew(pv) = Vm0(pv) .* Vnew(pv) ./ abs(Vnew(pv));
  ...
  % update Sbus for pv buses
  Sbus(pv) = real(Sbus(pv)) + 1j * imag( V(pv) .* conj(Ybus(pv,:) * V));
  % calculate actual power and mismatch
  Ibus = Ybus * V;
  S = V .* conj(Ibus);
  Smis = S - Sbus;
